...
function requestMethodToActionKey(key:string, requestMethod:string = defaultRequestType):string {
  ...
  const requestMethodConfig:RequestMethodConfig = container.getDefinition('requestMethods').getArgument(finalRequestMethod);
...
function actionFromMapping(mapping:Mapping):{name:string, fn:Function} {
  ...
  const actionFn = (...args):?RequestAction => {
    const finalRequest = typeof finalRequestFn === 'function' ? finalRequestFn(...args) : finalRequestFn;

    const action = (finalRequest.force || !container.getUtil('equals')(comparisonCache[actionKey], finalRequest.comparison)) ?
      reactReduxFetchActions
        .for(requestMethod)
        .request(Object.assign({}, mapping, {
...
export default function buildActionsFromMappings(mappings:Array<Mapping>):Object {
  const actionsBuilder = reduce((actions:Object, mapping:Mapping) => {
